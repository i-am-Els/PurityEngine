...
namespace isle_engine::math
{
    ...
    public:
        ...
        Quaternion(T angle, const Vector3<T>& axis, bool isEuler = true);
        ~Quaternion() = default;
    ...
    template<class T>
    Quaternion<T>::Quaternion(T angle, const Vector3<T>& axis, bool isEuler){
    ...
    Quaternion<T> Quaternion<T>::s_createIdentity() {
        return Quaternion<T>(T(1), Vector3<T>::zero, false);
    }
